...
console.log(`ready. total=${total}ms  sequence chunks loaded: ${netLog.length} requests so far`);
...
await browser.close();
const ok = allCompleted && allIdentical && netDuringPlay === 0 && consoleErrors.length === 0;
process.exit(ok ? 0 : 1);
