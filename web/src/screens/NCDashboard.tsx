...
type LayerId = "climate" | "gauge" | "flow" | "alert" | "storm" | "fire" | "camera" | "closure";

const LAYER_META: Record<LayerId, { label: string; note: string }> = {
  climate: { label: "Current conditions", note: "ECONet + RAWS (CLOUDS API)" },
  gauge: { label: "River flood forecast", note: "NOAA NWPS / National Water Model" },
...
const LAYER_ORDER: LayerId[] = [
  "gauge", "flow", "alert", "climate", "fire", "storm", "closure", "camera",
];
...
// cameras + closures sit off the flood-severity ramp, so the legend names them
const CAMERA_RGB: [number, number, number] = [139, 148, 163];
const CLOSURE_RGB: [number, number, number] = [232, 137, 74];

const sevRGB = (s: number): [number, number, number] => NC_SEV_COLOR[Math.max(-1, Math.min(3, s))];
...
export default function NCDashboard({ onOpenAssess, onOpenAbout }: Props) {
  ...
  const { containerRef, mapRef, ready, styleEpoch } = useMapLibre({ center: [-78.7, 35.4], zoom: 6.7 });
  ...
  const [selected, setSelected] = useState<
    { title: string; detail?: string; source: string; coord: [number, number]; url?: string } | null
  >(null);
        ...
        map.addLayer(
          {
            ...
            type: "raster",
            source: id,
            paint: { "raster-opacity": i === radarIdx ? 0.5 : 0, "raster-fade-duration": 120 },
          },
          firstSymbol,
        );
      });
  ...
  useEffect(() => {
    ...
    try {
      ...
      radarFrames.forEach((_, i) => {
        const id = `nc-radar-${i}`;
        if (map.getLayer(id)) map.setPaintProperty(id, "raster-opacity", i === radarIdx ? 0.5 : 0);
      });
    } catch {
      /* style mid-swap */
    }
  ...
  useEffect(() => {
    ...
    const frame = () => {
      ...
      map.fitBounds(
        [NC_BBOX[0], NC_BBOX[1], NC_BBOX[2], NC_BBOX[3]] as [number, number, number, number],
        { padding: { top: 48, bottom: 48, left: side, right: side }, duration: 0 },
      );
    };
    frame();
  ...
  const pointLayers = useMemo(() => {
    ...
    // point layers. cameras + closures aren't on the hazard-severity ramp,     // they get their own fixed marks so the ramp still means "flood risk".
    const FLOW_RGB: [number, number, number] = [90, 150, 200];
    for (const id of ["gauge", "flow", "climate", "fire", "closure", "camera"] as const) {
      const res = results[id];
      if (!on[id] || !res || res.disabled) continue;
      ...
      out.push(
        new ScatterplotLayer({
          ...
          filled: !isCam,
          radiusUnits: "pixels",
          getPosition: (f) => (f as PF).geometry.coordinates as [number, number],
          getRadius: (f) => {
            if (isCam) return 2.5;
          ...
          onClick: (info: PickingInfo) => {
            ...
            setSelected({
              ...
              detail:
                f.properties.detail ??
                [f.properties.value, f.properties.category].filter(Boolean).join(" · "),
              source: LAYER_META[id].note,
              coord: c,
              url: f.properties.url,
            });
    ...
    return out;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [climate, gauge, flow, alert, storm, fire, camera, closure, on]);
  ...
  const focusPoint = useMemo<[number, number] | null>(() => {
    ...
    const fireStation = (climate?.data.features ?? []).find((f) => f.properties.severity >= 2);
    if (fireStation) return fireStation.geometry.coordinates as [number, number];
  ...
  const nearest = (feats: NCPointFC["features"], n: number, maxKm: number) => {
    if (!focusPoint || !feats.length) return [] as { c: NCPointFC["features"][number]; km: number }[];
  ...
  const anyStale = (["gauge", "flow", "alert", "climate"] as const).some((id) => {
    const r = results[id];
  ...
  const riskPanels = (
    <>
      <section className="panel px-3.5 py-3.5">
        ...
        {climate?.disabled ? (
          <p className="text-2xs leading-relaxed text-ink-faint">{climate.reason}</p>
        ) : climate ? (
          <>
            <StatTile
            ...
            />
            {(() => {
              const fire = (climate.data.features ?? []).filter((f) => f.properties.severity >= 2);
              return fire.length ? (
                <p className="mt-2 rounded-sm bg-dmg1/15 px-1.5 py-1 text-2xs text-dmg1">
                  {fire.length} RAWS station{fire.length > 1 ? "s" : ""} at fire-weather risk
                  (low fuel moisture + wind)
                </p>
              ) : null;
            })()}
          </>
      ...
      <section className="panel px-3.5 py-3.5">
        ...
        {alert ? (
          <RankedList as="ul" items={alertItems} empty="No active NWS alerts for North Carolina." />
        ) : (
      ...
      {fire && !fire.disabled && fire.data.features.length > 0 && (
        <section className="panel px-3.5 py-3.5">
          ...
          <p className="text-2xs leading-relaxed text-ink-faint">
            ...
            {fire.data.features.length > 1 ? "s" : ""} in North Carolina in the past 24 h
            (NASA FIRMS). Thermal anomalies, not all are wildfires.
          </p>
        </section>
      )}
              ...
              <ul className="mt-2 space-y-1">
                {history.recent.map((d) => (
                  <li key={d.disasterNumber} className="text-2xs text-ink-dim">
                    <span className="tnum text-ink-faint">{d.declarationDate.slice(0, 4)}</span>{" "}
                    {d.declarationTitle}
                  </li>
                ))}
              </ul>
              {history.stale && (
                <p className="mt-1.5 text-2xs text-ink-faint">{history.source}</p>
              )}
            </>
          ) : (
          ...
          <ul className="space-y-1">
            {(storm.data.features ?? []).map((f) => (
              <li key={f.properties.id} className="text-xs text-ink">
                {f.properties.title}
                {f.properties.detail && (
                  <span className="block text-2xs text-ink-faint">{f.properties.detail}</span>
                )}
              </li>
            ))}
          </ul>
          <p className="mt-1.5 text-2xs text-ink-faint">
            NHC forecast cone, track line and watch/warning zones (committed snapshot).
          </p>
        </section>
      )}

      {elevatedRisk && (nearbyCameras.length > 0 || nearbyClosures.length > 0) && (
        <section className="panel px-3.5 py-3.5">
          <h2 className="section-title mb-1.5">Ground truth near the risk</h2>
          {nearbyClosures.length > 0 && (
            <div className="mb-2">
              <p className="mb-1 text-2xs font-medium text-ink-dim">Road closures</p>
              <RankedList
                items={nearbyClosures.map(({ c, km }) => ({
                  ...
                  onSelect: () => flyTo(c.geometry.coordinates as [number, number], 12),
                }))}
              />
            </div>
          )}
          {nearbyCameras.length > 0 && (
            <>
              <p className="mb-1 text-2xs font-medium text-ink-dim">Traffic cameras</p>
              <RankedList
  ...
  return (
    <div className="relative h-full">
      <div ref={containerRef} className="h-full w-full" aria-label="North Carolina risk dashboard" />
      ...
      <aside className="absolute left-3 top-3 z-20 flex max-h-[calc(100%-1.5rem)] w-[18rem] max-w-[calc(100vw-1.5rem)] flex-col gap-3 overflow-y-auto sm:left-4 sm:top-4">
        ...
        <section className="panel px-4 py-3.5">
          ...
          <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1.5 border-t border-line pt-2.5">
            {[-1, 0, 1, 2, 3].map((s) => (
              <span key={s} className="flex items-center gap-1.5 text-2xs text-ink-faint">
                <span className="h-2 w-2 rounded-full" style={{ background: sevCSS(s) }} />
                {NC_SEV_LABEL[s + 1]}
              </span>
            ))}
            <span className="flex items-center gap-1.5 text-2xs text-ink-faint">
            ...
            <span className="flex items-center gap-1.5 text-2xs text-ink-faint">
              <span
                className="h-2 w-2 rounded-full"
                style={{ boxShadow: `inset 0 0 0 1.5px rgb(${CAMERA_RGB.join(",")})` }}
              />
              camera
            </span>
          </div>
      ...
      {/* mobile: a compact state strip that opens the full sheet */}
