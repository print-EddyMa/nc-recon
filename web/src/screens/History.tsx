...
const styleFor = (t: string) => TYPE_STYLE[t] ?? { color: "#8a8f98", short: t };
...
const shortTitle = (s: string) =>
  cleanTitle(s).replace(
    /^(Hurricane|Tropical Storm|Tropical Depression)\s+/i,
    "",
  );
...
export default function History({ areas, onOpenAssess, onOpenArea }: Props) {
  ...
  const rows = useMemo(() => {
    const list = (history?.all ?? [])
      ...
      .slice()
      .sort(
        (a, b) => Date.parse(a.declarationDate) - Date.parse(b.declarationDate),
      );
    ...
    return list.map((d) => {
      ...
      const w = labelled
        ? Math.max(44, shortTitle(d.declarationTitle).length * 6)
        : 10;
      let lane = 0;
      if (labelled) {
  ...
  const ticks = useMemo(() => {
    ...
    for (let y = y0; y <= y1; y++) {
      out.push({
        x: ((Date.UTC(y, 0, 1) - START) / DAY) * pxPerDay,
        label: String(y),
        major: true,
      });
      if (monthly) {
    ...
    <div className="flex h-full flex-col overflow-y-auto lg:flex-row lg:overflow-hidden">
      ...
      <div className="flex min-h-0 min-w-0 shrink-0 flex-col px-5 py-4 md:px-8 lg:flex-1">
        <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
          <div>
            <p className="cap text-ink-faint">
              North Carolina · federally-declared disasters
            </p>
            <h1 className="font-display text-xl font-semibold text-ink">
              {history
                ? `${history.total} declarations since ${history.sinceYear}`
                : "Disaster history"}
            </h1>
          </div>
          <div className="flex items-center gap-1">
            {presets.map(([label, v]) => (
              <button
                ...
                className={`pressable rounded-md border px-2 py-1 text-2xs ${Math.abs(pxPerDay - v) < 0.02 ? "border-accent bg-accent/10 text-ink" : "border-line text-ink-dim hover:text-ink"}`}
              >
                {label}
              </button>
            ))}
        ...
        <p className="mt-1 text-2xs text-ink-faint">
          Scroll to pan, scroll-wheel over the track to zoom. Click an event to
          open it.
        </p>
                ...
                >
                  ...
                  <span
                    ...
                    style={{
                      ...
                      background: st.color,
                      boxShadow: sel
                        ? "0 0 0 3px rgb(var(--accent) / 0.4)"
                        : undefined,
                    }}
                  />
                  {labelled && (
                    <span
                      className={`absolute left-1/2 -translate-x-1/2 whitespace-nowrap rounded px-1 text-2xs ${sel ? "bg-accent text-accent-ink" : "bg-surface text-ink-dim group-hover:text-ink"}`}
                      style={{ top: -stickH - 17 }}
                    >
        ...
        <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1">
          {[...new Set((history?.all ?? []).map((d) => d.incidentType))]
            .slice(0, 8)
            .map((t) => (
              <span
                key={t}
              ...
              >
                <span
                  ...
                  style={{ background: styleFor(t).color }}
                />
                {styleFor(t).short}
              </span>
            ))}
        ...
        {history && history.byType.length > 0 && (
          <div className="mt-4 grid gap-x-10 gap-y-4 border-t border-line pt-4 md:grid-cols-[1fr_minmax(15rem,20rem)]">
            <p className="measure text-xs leading-relaxed text-ink-dim">
              ...
              federal disaster declarations. Events from 2016 on can be run
              through the damage pipeline; older ones open the archived NWS
              radar for that day (the archive reaches back to 1995).
            </p>
            <div>
              <p className="cap mb-1.5">Top 6 types by count</p>
              <dl className="space-y-1.5">
                {history.byType.slice(0, 6).map((t) => {
                  ...
                  return (
                    ...
                    >
                      <dt
                        className="w-24 shrink-0 truncate text-ink-dim"
                        title={t.type}
                      >
                      ...
                      <dd className="bar-track relative m-0 h-2.5 flex-1">
                        <span
                          ...
                          style={{
                            width: `${Math.max(w, 3)}%`,
                            background: styleFor(t.type).color,
                          }}
                        />
                      </dd>
...
function EventDetail({
  ...
  const showFemaNote =
    !!eventName && eventName.toLowerCase() !== femaTitle.toLowerCase();
    ...
    <div>
      ...
      {showFemaNote && (
        <p className="mt-0.5 text-2xs text-ink-faint">
          FEMA declaration title: {femaTitle}
        </p>
      )}
      ...
      <div className="mt-4 border-t border-line pt-3">
        {imgEra ? (
            ...
            {matches.length > 0 ? (
              <>
                <p className="text-xs leading-relaxed text-ink-dim">
                  {matches.length === 1
                    ? "One area has"
                    : `${matches.length} areas have`}{" "}
                  been assessed for this event.
                </p>
              ...
              <>
                <p className="text-xs leading-relaxed text-ink-dim">
                  This is recent enough that post-event Maxar Open Data imagery
                  likely exists. Run the damage pipeline on an affected North
                  Carolina area.
                </p>
                <button
          ...
          <>
            ...
            <HistoricalRadar
              dateStr={d.incidentBeginDate ?? d.declarationDate}
            />
          </>
        )}
      </div>
