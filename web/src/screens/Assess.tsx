...
function centerOf(ev: CatalogEvent): [number, number] | null {
  ...
  if (ev.bbox)
    return [(ev.bbox[0] + ev.bbox[2]) / 2, (ev.bbox[1] + ev.bbox[3]) / 2];
  return null;
}
...
export default function Assess({
  ...
  const effEventId = ranked.some((e) => e.id === eventId)
    ? eventId
    : (ranked[0]?.id ?? "");
  ...
  const effName =
    name.trim() ||
    (point
      ? `NC AOI ${point[1].toFixed(3)}, ${point[0].toFixed(3)}`
      : "NC AOI");
  ...
  return (
    <div className="relative h-full">
      <div
        ...
        aria-label="Pick an area to assess"
      />

      {/* hidden below sm: at narrow widths this wraps tall enough to run under
          the aside panel (which already restates "click the map" for a first
          AOI), so it's redundant there rather than merely decorative */}
      <div className="absolute left-1/2 top-3 z-20 hidden w-[min(720px,calc(100%-1.5rem))] -translate-x-1/2 sm:block">
        <div className="panel flex items-start gap-2.5 px-3.5 py-2.5 text-xs leading-relaxed text-ink-dim">
          ...
          <span>
            ...
            North Carolina to drop an AOI, pick the Maxar event that covers it,
            and run the damage pipeline for that footprint.{" "}
            <button
              onClick={onBack}
      ...
      <aside className="absolute right-3 top-3 z-20 flex max-h-[calc(100%-1.5rem)] w-[22rem] max-w-[calc(100vw-1.5rem)] flex-col gap-3 overflow-y-auto sm:top-16 sm:max-h-[calc(100%-5rem)] lg:right-4">
        <section className="panel px-3.5 py-3.5">
          ...
          {point && outside && (
            <p className="rounded-sm bg-dmg2/15 px-2 py-1.5 text-2xs text-dmg1">
              That point is outside North Carolina. NC Recon only assesses NC
              areas.
            </p>
          )}
            ...
            <div className="space-y-2.5">
              <label className="block">
                ...
                <input
                  ...
                  placeholder={effName}
                  className="w-full rounded-md border border-line bg-surface-2 px-2 py-1.5 text-xs text-ink outline-none focus-visible:border-accent"
                />
              </label>

              <label className="block">
                <span className="cap mb-1 block">
                  Maxar event covering this point
                </span>
                <select
                  value={effEventId}
                  onChange={(e) => setEventId(e.target.value)}
                  className="w-full rounded-md border border-line bg-surface-2 px-2 py-1.5 text-xs text-ink-dim outline-none focus-visible:border-accent"
                >
                  {ranked.length === 0 && (
                    <option value="">
                      no NC-covering events in the catalogue
                    </option>
                  )}
                  {ranked.map((e) => {
                    const c = centerOf(e)!;
                    return (
                      <option key={e.id} value={e.id}>
                        {e.name} · {distKm(point, c).toFixed(0)} km ·{" "}
              ...
              {chosen && (
                <p className="text-2xs leading-relaxed text-ink-faint">
                  ...
                  {coverage &&
                    ` ${coverage.n_cells} tiles with before + after imagery.`}
                </p>
              )}

              {coverage && !pointOk && (
                <p className="rounded-sm bg-dmg1/15 px-2 py-1.5 text-2xs text-dmg1">
                  This spot is outside the event&rsquo;s before/after coverage
                  (shaded on the map). Move the marker into a shaded tile.
                </p>
              )}
